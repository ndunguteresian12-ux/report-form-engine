    ...
    <body class="bg-slate-100 min-h-screen p-4 sm:p-8">
        <div class="max-w-xl mx-auto space-y-4">
            ...
            <form action="/api/v1/timetable/assignments/{school_id}" method="post" class="space-y-1">
                ...
                <input type="hidden" name="stream" value="{esc(stream)}">
                <input type="hidden" name="plan_id" value="{resolved_plan_id}">
                {rows_html or (f"<p class='text-slate-400 text-xs italic'>No subjects defined for \"{esc(stream)}\" yet. <a href='/timetable/combinations/{school_id}?grade_name={urllib.parse.quote(grade_name)}&stream={urllib.parse.quote(stream)}' class='text-indigo-700 font-bold hover:underline'>Set up this combination's subjects →</a></p>" if education_level == "Senior School" else "<p class='text-slate-400 text-xs italic'>No subjects configured for this education level.</p>")}
